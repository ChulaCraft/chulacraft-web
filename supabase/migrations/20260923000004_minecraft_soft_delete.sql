-- is_active = the player still owns this row (false = removed from their
-- dashboard). desired_whitelisted stays the whitelist state an admin controls.
-- The column already exists in production (added by hand); this records it.

alter table public.minecraft_registrations add column if not exists is_active boolean not null default true;
grant select (is_active) on public.minecraft_registrations to authenticated;
-- Production had direct write grants; every write goes through the RPCs.
revoke insert, update, delete on public.minecraft_registrations from anon, authenticated;

-- Hand-made, empty, unused. Chula SSO links live in cu_sso_identities.
-- Refuse to drop it if rows appeared since, instead of losing them.
do $$
begin
  if to_regclass('public.cu_registrations') is not null then
    if exists (select 1 from public.cu_registrations) then raise exception 'cu_registrations is not empty; migrate its rows first'; end if;
    drop table public.cu_registrations;
  end if;
end;
$$;
...
begin
  ...
  perform public.remove_minecraft_account(v_row.id);
  return query select * from public.add_minecraft_account(p_minecraft_uuid, p_minecraft_username);
end;
$$;

-- Player "delete": hide the row and drop it from the whitelist. Kept for audit
-- and so re-adding the same Mojang account restores it.
create or replace function public.remove_minecraft_account(p_registration_id uuid)
...
$$;
revoke all on function public.remove_minecraft_account(uuid) from public, anon;
grant execute on function public.remove_minecraft_account(uuid) to authenticated;
